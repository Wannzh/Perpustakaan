...
const LoansBooksSiswa: React.FC = () => {
    ...
    const [selectedLoanForRating, setSelectedLoanForRating] = useState<BookLoan | null>(null); // Changed to BookLoan
    ...
    const handleRatingSubmit = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/api/peminjaman/${selectedLoanForRating.id}/rating`, {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.message || `Gagal memberikan rating - Status ${response.status}`);
            }
        ...
        <div className="container mx-auto px-6 py-12 bg-gradient-to-br from-gray-100 to-blue-50 min-h-screen font-sans antialiased">
                        ...
                        >
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between">
                                <div className="flex-1">
                                    <h3 className="text-2xl font-bold text-gray-800 mb-4 tracking-tight line-clamp-1">{item.judulBuku}</h3>
                                    <div className="space-y-3 text-base text-gray-600">
                                        ...
                                        <div className="flex items-center">
                                            ...
                                            <span className="ml-2">{item.statusPengembalian || "Belum dikembalikan"}</span>
                                        </div>
                                        <div className="flex items-center">
                                            <CalendarIcon className="h-5 w-5 mr-3 text-blue-500" />
                                            <span className="font-semibold">Tanggal Pinjam:</span>
                                            <span className="ml-2">{new Date(item.tanggalPinjam).toLocaleDateString()}</span>
                                        </div>
                                        <div className="flex items-center">
                                            <CalendarIcon className="h-5 w-5 mr-3 text-blue-500" />
                                            <span className="font-semibold">Tanggal Kembali:</span>
                                            <span className="ml-2">{new Date(item.tanggalKembali).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                </div>
                                {item.status === "DIKEMBALIKAN" ? (
                                    <div className="mt-6 sm:mt-0 flex items-center gap-4">
                                        <span className="text-green-600 font-semibold text-base">Buku Telah Dikembalikan</span>
                                        <button
                                        ...
                                        >
                                            ...
                                            {item.rating ? `Sudah Diberi Rating (${item.rating}⭐)` : "Beri Rating"}
                                        </button>
                ...
                >
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-3xl font-bold text-gray-800 tracking-tight">Beri Rating Buku</h2>
                            <button
                        ...
                        <div className="text-center mb-8">
                            <h3 className="text-xl font-semibold text-gray-700 mb-3">{selectedLoanForRating.judulBuku}</h3>
                            <p className="text-base text-gray-500">Bagaimana pengalaman Anda dengan buku ini?</p>
                        </div>
                        <div className="flex justify-center mb-8">
                            <div className="flex gap-4">
                                {[1, 2, 3, 4, 5].map((star) => (
                                    ...
                                    >
                                        <Star
                                            className={`h-10 w-10 ${star <= (hoverRating || rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"
                        ...
                        <div className="flex gap-4">
                            ...
                            <button
                                className={`flex-1 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white py-3 px-6 rounded-lg hover:from-yellow-600 hover:to-yellow-700 transition-all duration-300 transform hover:scale-105 shadow-md ${rating === 0 || isSubmittingRating ? "opacity-50 cursor-not-allowed" : ""
                                    }`}
            ...
            {selectedLoan && !showReturnModal && !showRatingModal && (
                ...
                >
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-3xl font-bold text-gray-800 tracking-tight">Detail Peminjaman</h2>
                            <button
                                className="text-gray-500 hover:text-gray-700 transition-colors duration-200 transform hover:scale-110"
                                onClick={closeDetailModal}
                            >
                        ...
                        <div className="space-y-4 text-base text-gray-600">
                            ...
                            <div className="flex items-center">
                                <TagIcon className="h-6 w-6 mr-3 text-blue-500" />
                                <span className="font-semibold">Status Pengembalian:</span>
                                <span className="ml-2">{selectedLoan.statusPengembalian || "Belum dikembalikan"}</span>
                            </div>
                            <div className="flex items-center">
                                <CalendarIcon className="h-6 w-6 mr-3 text-blue-500" />
                                <span className="font-semibold">Tanggal Pinjam:</span>
                                <span className="ml-2">{new Date(selectedLoan.tanggalPinjam).toLocaleDateString()}</span>
                            </div>
                            <div className="flex items-center">
                                <CalendarIcon className="h-6 w-6 mr-3 text-blue-500" />
                                <span className="font-semibold">Tanggal Kembali:</span>
                                <span className="ml-2">{new Date(selectedLoan.tanggalKembali).toLocaleDateString()}</span>
                            </div>
                        </div>
                        <div className="mt-8">
                            <button
            ...
            {selectedLoan && showReturnModal && (
                ...
                >
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-3xl font-bold text-gray-800 tracking-tight">Kembalikan Buku</h2>
                            <button
                                className="text-gray-500 hover:text-gray-700 transition-colors duration-200 transform hover:scale-110"
                                onClick={closeReturnModal}
                            >
                        ...
                        <div className="space-y-6 text-base text-gray-600">
                            ...
                            <div>
                                <label className="block text-base font-semibold text-gray-700 mb-2">Status Pengembalian:</label>
                                <select
                            ...
                            <div>
                                <label className="block text-base font-semibold text-gray-700 mb-2">Catatan (Opsional):</label>
                                <textarea
                                    className="w-full p-4 border border-gray-200 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300 bg-gray-50"
                                    rows={4}
                                    value={catatan}
                        ...
                        <div className="mt-8 flex gap-4">
                            ...
                            <button
                                className={`flex-1 bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3 px-6 rounded-lg hover:from-blue-700 hover:to-blue-800 transition-all duration-300 transform hover:scale-105 shadow-md ${isReturning ? "opacity-50 cursor-not-allowed" : ""
                                    }`}
            ...
            {showPopup && returnMessage && (
                ...
                >
                    ...
                    >
                        ...
                        <p className={`text-base ${returnMessage.includes("Error") ? "text-red-500" : "text-green-500"}`}>{returnMessage}</p>
                        <button
